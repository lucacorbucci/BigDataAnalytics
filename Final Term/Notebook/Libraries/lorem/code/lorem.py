...
from Libraries.lorem.code.rule import Rule, compact_premises

from Libraries.lorem.code.explanation import Explanation, MultilabelExplanation
from Libraries.lorem.code.decision_tree import learn_local_decision_tree
from Libraries.lorem.code.neighgen import RandomGenerator, GeneticGenerator, RandomGeneticGeneticGenerator, ClosestInstancesGenerator
from Libraries.lorem.code.rule import get_rule, get_counterfactual_rules
from Libraries.lorem.code.util import calculate_feature_values, neuclidean, multilabel2str, multi_dt_predict
...
class LOREM(object):
    ...
    def __init__(self, K, bb_predict, feature_names, class_name, class_values, numeric_columns, features_map,
                 ...
                 size=1000, ocr=0.1, multi_label=False, one_vs_rest=False, kernel_width=None, kernel=None,
    ...
    def __explain_tabular_instance_single_tree(self, x, Z, Yb, weights):
        ...
        crules, deltas = get_counterfactual_rules(x, Yc[0], dt, Z, Yc, self.feature_names, self.class_name,
                                                  self.class_values, self.numeric_columns, self.multi_label)
    ...
    def __init_neighbor_fn(self, ocr, categorical_use_prob, continuous_fun_estimation, size, kwargs):
        ...
        if self.neigh_type in ['random', 'genetic', 'rndgen']:
            if self.verbose:
        ...
        if self.neigh_type in ['genetic', 'rndgen']:
            alpha1 = kwargs.get('alpha1', 0.5)
            alpha2 = kwargs.get('alpha2', 0.5)
            eta1 = kwargs.get('eta1', 0.7)
            eta2 = kwargs.get('eta2', 1.0)
            ...
            if self.neigh_type == 'genetic':
                neighgen = GeneticGenerator(self.bb_predict, self.feature_values, self.features_map, nbr_features,
                                            ...
                                            alpha2=alpha2, eta1=eta1, eta2=eta2, metric=metric, ngen=ngen,
                                            mutpb=mutpb, cxpb=cxpb, tournsize=tournsize,
                                            halloffame_ratio=halloffame_ratio, random_seed=random_seed,
                                            verbose=self.verbose)
            else:  # neigh_type == 'rndgen':
                neighgen = RandomGeneticGeneticGenerator(self.bb_predict, self.feature_values, self.features_map,
                                                         nbr_features, nbr_real_features, numeric_columns_index,
                                                         ocr=ocr, alpha1=alpha1, alpha2=alpha2, eta1=eta1, eta2=eta2,
            ...
            k = kwargs.get('k', None)
            type = kwargs.get('core_neigh_type', 'unified')
