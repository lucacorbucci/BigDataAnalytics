...
def my_prepare_dataset(df):
    ...
    rdf = df
    df, feature_names = my_one_hot_encoding(df)

    real_feature_names = my_get_real_feature_names(rdf, numeric_columns)

    rdf = rdf[real_feature_names]

    features_map = get_features_map(feature_names, real_feature_names)

    return df, feature_names, numeric_columns, rdf, real_feature_names, features_map


def my_one_hot_encoding(df):

    dfX = pd.get_dummies(df[[c for c in df.columns]], prefix_sep='=')
    df = dfX
    feature_names = list(dfX.columns)
    return df, feature_names


def my_get_real_feature_names(rdf, numeric_columns):
    real_feature_names = [c for c in rdf.columns if c in numeric_columns]
    real_feature_names += [c for c in rdf.columns if c not in numeric_columns]
    return real_feature_names
